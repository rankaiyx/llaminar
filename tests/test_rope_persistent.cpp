...
static double max_abs_diff(const std::vector<float>& a, const std::vector<float>& b){
    double m=0.0; for(size_t i=0;i<a.size();++i){ m = std::max(m, std::abs((double)a[i]-b[i])); } return m; }

TEST_F(RopeFixture, IncrementalParitySequentialSteps){
    const int head_dim=128; const int q_heads=8; const int k_heads=8; const float freq_base=10000.f; const int steps=128;
    ...
    for(int n=0;n<steps;++n){
        ...
        std::vector<float> q_persist = q_seed; std::vector<float> k_persist = k_seed;
        std::vector<float> q_ref = q_seed;     std::vector<float> k_ref = k_seed;
...
TEST_F(RopeFixture, RestartResetsPersistentState){
    const int head_dim=64; const int q_heads=4; const int k_heads=4; const float freq_base=10000.f;
    std::vector<float> q0(head_dim*q_heads), k0(head_dim*k_heads);
    std::vector<float> q1(head_dim*q_heads), k1(head_dim*k_heads);
    fill_seed(q0,k0); fill_seed(q1,k1);
    // Advance some steps
    for(int n=0;n<32;++n){ apply_rope_experimental(q0.data(), k0.data(), 1, head_dim, q_heads, k_heads, n, freq_base); }
    ...
    std::vector<float> q_ref = q0; std::vector<float> k_ref = k0; // pull original? need fresh seed
...
TEST_F(RopeFixture, LargeJumpFastPathParity){
    const int head_dim=128; const int q_heads=4; const int k_heads=4; const float freq_base=10000.f;
    std::vector<float> qA(head_dim*q_heads), kA(head_dim*k_heads);
    std::vector<float> qB(head_dim*q_heads), kB(head_dim*k_heads);
    fill_seed(qA,kA); fill_seed(qB,kB);
    ...
    std::vector<float> q_ref(head_dim*q_heads), k_ref(head_dim*k_heads); fill_seed(q_ref,k_ref);
