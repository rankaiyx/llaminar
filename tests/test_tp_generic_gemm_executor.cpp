...
static bool ref_matmul(const float* A, const float* B, float* C, std::size_t M, std::size_t N, std::size_t K) {
    ...
    for (std::size_t m=0;m<M;++m) {
        for (std::size_t k=0;k<K;++k) {
            ...
            float* crow = C + m*N;
            for (std::size_t n=0;n<N;++n) crow[n] += a * brow[n];
        }
    }
    return true;
}

struct Case { std::size_t M,N,K; int tp_size; TPGemmExecConfig::Mode mode; };
...
TEST_P(TPGemmExecutorParity, ParityVsReference) {
    ...
    for (int r=0; r<c.tp_size; ++r) {
        TPGemmExecConfig cfg; cfg.mode = c.mode; cfg.tp_size = c.tp_size; cfg.tp_rank = r;
