...
TEST(Perf__Q2_K_Unpack, SIMD_Tier_Benchmark)
{
    const int num_blocks = 10000;
    const int iterations = 1000;
    const int warmup = 100;

    std::vector<Q2_KBlock> blocks(num_blocks);
    ...
    std::cout << "\n=== Q2_K Unpack Performance ===" << std::endl;

    double scalar_time = benchmark([&](int i) {
        unpack_q2_k_superblock_to_int8_scalar(blocks[i], output.data() + i * 256, scales.data() + i * 8, mins.data() + i * 8);
        asm volatile("" ::: "memory");
    }, "Scalar");
    ...
    if (cpu_supports_avx2())
    {
        avx2_time = benchmark([&](int i) {
            unpack_q2_k_superblock_to_int8_avx2(blocks[i], output.data() + i * 256, scales.data() + i * 8, mins.data() + i * 8);
            asm volatile("" ::: "memory");
        }, "AVX2");
    }
#endif
    ...
    if (cpu_supports_avx512())
    {
        avx512_time = benchmark([&](int i) {
            unpack_q2_k_superblock_to_int8_avx512(blocks[i], output.data() + i * 256, scales.data() + i * 8, mins.data() + i * 8);
            asm volatile("" ::: "memory");
        }, "AVX-512");
    }
#endif
