...
inline int MockModelLoader::getInt(const std::string& key, int default_val) const {
    ...
    if (it != int_params_.end()) return it->second;

    // Check known hyperparameters
    if (key == "block_count") return static_cast<int>(block_count_);
    if (key == "head_count") return static_cast<int>(head_count_);
    if (key == "head_count_kv") return static_cast<int>(head_count_kv_);
...
inline uint64_t MockModelLoader::getUInt64(const std::string& key, uint64_t default_val) const {
    // Check known hyperparameters
    if (key == "block_count") return block_count_;
    if (key == "embedding_length") return embedding_length_;
    if (key == "context_length") return context_length_;
    if (key == "head_count") return head_count_;
    if (key == "head_count_kv") return head_count_kv_;
    if (key == "vocab_size") return vocab_size_;

    // Check custom int params (promote to uint64)
    auto it = int_params_.find(key);
    if (it != int_params_.end()) return static_cast<uint64_t>(it->second);
...
inline float MockModelLoader::getFloat(const std::string& key, float default_val) const {
    ...
    if (it != float_params_.end()) return it->second;

    if (key == "rope_theta") return rope_theta_;
    if (key == "rms_norm_eps") return rms_norm_eps_;
...
inline std::string MockModelLoader::getString(const std::string& key, const std::string& default_val) const {
    ...
    if (it != string_params_.end()) return it->second;

    if (key == "architecture") return architecture_;
    ...
    WeightPrecision /*weight_precision*/)
{
    ...
    if (shape_it == tensor_shapes_.end()) return nullptr;
