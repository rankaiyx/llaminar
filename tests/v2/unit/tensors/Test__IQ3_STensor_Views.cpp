...
TEST_F(Test__IQ3_STensor_Views, View3DParentBasicSlice)
{
    ...
    auto tensor_3d = std::make_shared<IQ3_STensor>(std::vector<size_t>{K, R, N}, raw);
...
TEST_F(Test__IQ3_STensor_Views, View3DParentWithOffset)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
...
TEST_F(Test__IQ3_STensor_Views, View3DParentBoundsCheck)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
