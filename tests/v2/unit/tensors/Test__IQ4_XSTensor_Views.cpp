...
TEST_F(Test__IQ4_XSTensor_Views, View3DParentBasicSlice)
{
    ...
    std::vector<uint8_t> raw(total_blocks * sizeof(IQ4_XSBlock));
    auto tensor_3d = std::make_shared<IQ4_XSTensor>(std::vector<size_t>{K, R, N}, raw);
...
TEST_F(Test__IQ4_XSTensor_Views, View3DParentWithOffset)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
...
TEST_F(Test__IQ4_XSTensor_Views, View3DParentBoundsCheck)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
