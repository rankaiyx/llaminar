...
TEST(Test__Q4_KTensor, IBlockDecoderInterface)
{
    ...
    // Q4_K should support ITensorGemmTileDataProvider interface
...
TEST(Test__Q4_KTensor, View3DParentBasicSlice)
{
    ...
    auto tensor_3d = std::make_shared<Q4_KTensor>(std::vector<size_t>{K, R, N}, raw);
...
TEST(Test__Q4_KTensor, View3DParentWithOffset)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
...
TEST(Test__Q4_KTensor, View3DParentBoundsCheck)
{
    const size_t N = 4, R = 8, K = 512;
    const size_t blocks_per_row = K / 256;
    const size_t total_blocks = N * R * blocks_per_row;
