/**
 * @file Test__Qwen2_LocalTP_NCCL_vs_PyTorch.cpp
 * @brief Integration: True LOCAL TP Parity Test with MultiDeviceOrchestrator
 *
 * This test performs ACTUAL LOCAL tensor parallelism with:
 *   - 2 CUDA devices (cuda:0, cuda:1)
 *   - Weight sharding via MultiDeviceOrchestrator
 *   - NCCL backend for collective operations
 *   - PyTorch reference comparison
 *
 * This is distinct from Test__Qwen2_TP_NCCL_vs_PyTorch.cpp which only tests
 * single-device CUDA inference with NCCL availability check.
 *
 * Key differences from the existing NCCL parity test:
 ...
 *   - Uses NCCL AllReduce for collective operations
 *
 * Test requirements:
...
// Note: We do NOT include <nccl.h> directly here because:
// 1. The RCCL build's nccl.h includes HIP headers which conflict with CUDA headers
// 2. We only need the HAVE_NCCL macro to check backend availability
// 3. The actual NCCL calls are handled internally by LocalTPContext
...
/**
 * @brief Test fixture for TRUE LOCAL TP NCCL parity testing
 *
 * Uses MultiDeviceOrchestrator to perform actual tensor parallelism
 * across 2 CUDA GPUs with NCCL as the collective backend.
 */
...
protected:
    ...
    {
        // LOCAL TP has slightly higher divergence due to:
        // - Weight sharding introduces numerical differences
        // - AllReduce operations have finite precision
        // - Column-parallel outputs are combined via NCCL
        //
        // Thresholds are relaxed compared to single-device CUDA:
        //   - Single-device CUDA: cosine >= 0.95, KL < 0.10
        //   - LOCAL TP (2 CUDA): cosine >= 0.90, KL < 0.15
        return BackendThresholds{
            .cosine_threshold = 0.90f, // Relaxed for LOCAL TP
            .decode_cosine_threshold = 0.90f,
            .early_layers_count = 6,
            .min_early_layers_passed = 4, // Allow 2 early layer failures for TP
            .kl_threshold = 0.20f,        // Relaxed KL threshold
            .excluded_stages = {
    ...
    {
        return "LOCAL_TP_NCCL(2xCUDA)";
    }

    DeviceId getDeviceForRank() override
    {
        // For LOCAL TP, the "primary" device for snapshot comparison is cuda:0.
    ...
    void SetUp() override
    {
        // Check for CUDA GPUs
#ifdef HAVE_CUDA
        if (cudaGetDeviceCount(&cuda_count_) != cudaSuccess)
        {
            cuda_count_ = 0;
        }
        ...
        if (cuda_count_ >= 2)
        {
            ...
            LOG_INFO("[LocalTP NCCL Parity] Found " << cuda_count_ << " CUDA devices");
        }
        else
        {
            LOG_WARN("[LocalTP NCCL Parity] Need at least 2 CUDA devices (found " << cuda_count_ << ")");
        }
#else
        LOG_WARN("[LocalTP NCCL Parity] CUDA not compiled in (HAVE_CUDA not defined)");
#endif

        // Check NCCL availability
#ifdef HAVE_NCCL
        nccl_available_ = true;
        LOG_INFO("[LocalTP NCCL Parity] NCCL backend available");
#else
        LOG_WARN("[LocalTP NCCL Parity] NCCL not compiled in (HAVE_NCCL not defined)");
#endif
        ...
        LOG_INFO("╔══════════════════════════════════════════════════════════════════╗");
        LOG_INFO("║        TRUE LOCAL TENSOR PARALLELISM (NCCL) PARITY TEST          ║");
        LOG_INFO("╠══════════════════════════════════════════════════════════════════╣");
     ...
     * to create a multi-device orchestrator with NCCL backend.
     */
    bool setupLocalTPPipeline()
    {
        ...
        // Create LOCAL TP context with 2 CUDA devices
        std::vector<GlobalDeviceAddress> devices = {
            GlobalDeviceAddress::cuda(cuda_device_0_),
            GlobalDeviceAddress::cuda(cuda_device_1_)};
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, BackendSelection_IsNCCL)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
    ...
    LOG_INFO("[LocalTP NCCL Test] Group: " << group.toString());
    LOG_INFO("[LocalTP NCCL Test] Expected backend: NCCL (all CUDA, LOCAL scope)");
}

/**
 * @brief Test: Verify LocalTPContext creation with NCCL backend
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, LocalTPContext_Creation)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
    ...
    auto tp_ctx = createLocalTPContext(devices, {0.5f, 0.5f}, CollectiveBackendType::NCCL);
    ...
    EXPECT_EQ(tp_ctx->backend(), CollectiveBackendType::NCCL) << "Backend should be NCCL";
    ...
    LOG_INFO("[LocalTP NCCL Test] LocalTPContext created successfully");
}
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, PrefillParity_LocalTP)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, DecodeParity_LocalTP)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, SnapshotInfrastructure_LocalTP)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
    ...
    LOG_INFO("[LocalTP NCCL Test] Captured " << keys.size() << " snapshots");
}
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, LogitsSanityCheck_LocalTP)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
    ...
    LOG_INFO("[LocalTP NCCL Test] Logits sanity check passed:");
 ...
 */
TEST_F(Test__Qwen2_LocalTP_NCCL_vs_PyTorch, MultiToken_Sequence_LocalTP)
{
    if (!dual_cuda_available_ || !nccl_available_)
    {
        GTEST_SKIP() << "Hardware requirements not met";
    ...
    LOG_INFO("[LocalTP NCCL Test] Generated " << generated_tokens.size() << " tokens:");
