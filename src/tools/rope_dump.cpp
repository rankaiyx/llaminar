...
static void print_json(const std::vector<float>& q_in, const std::vector<float>& k_in,
    ...
    dump_array("q_input", q_in); std::cout << ",\n";
    dump_array("k_input", k_in); std::cout << ",\n";
    dump_array("q_rot", q_rot); std::cout << ",\n";
    dump_array("k_rot", k_rot); std::cout << "\n}" << std::endl;
}
...
int main(int argc, char** argv){
    ...
    for(int i=1;i<argc;++i){
        std::string a = argv[i];
        auto need = [&](const std::string& name){ if(i+1>=argc){ std::cerr<<"Missing value for "<<name<<"\n"; std::exit(1);} return std::string(argv[++i]);};
        if(a=="--seq") seq_len = std::stoi(need(a));
        else if(a=="--q-heads") q_heads = std::stoi(need(a));
        else if(a=="--k-heads") k_heads = std::stoi(need(a));
        else if(a=="--head-dim") head_dim = std::stoi(need(a));
        else if(a=="--n-past") n_past = std::stoi(need(a));
        else if(a=="--freq-base") freq_base = std::stof(need(a));
        else if(a=="--seed") seed = (unsigned)std::stoul(need(a));
        else if(a=="--input-json") input_json = need(a);
    }
    ...
    if(!input_json.empty()){
        ...
        if(!fin){ std::cerr << "Failed to open input JSON: "<<input_json<<"\n"; return 1; }
        ...
        if(!got_q || q_in.size()!=q_size){ std::cerr<<"Input JSON missing q_input/q or size mismatch (expected "<<q_size<<")\n"; return 1; }
        if(!got_k || k_in.size()!=k_size){ std::cerr<<"Input JSON missing k_input/k or size mismatch (expected "<<k_size<<")\n"; return 1; }
    } else {
        for(auto &x: q_in) x = dist(rng);
        for(auto &x: k_in) x = dist(rng);
    }
