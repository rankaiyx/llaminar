...
static bool parse_int(const char* s, int &out){
    if(!s) return false; out = std::atoi(s); return out>0; }

Args parse_args(int argc, char** argv){
    Args a; for(int i=1;i<argc;++i){
        if(!std::strcmp(argv[i],"--seq") && i+1<argc) parse_int(argv[++i], a.seq);
        else if(!std::strcmp(argv[i],"--heads") && i+1<argc) parse_int(argv[++i], a.heads);
        else if(!std::strcmp(argv[i],"--head-dim") && i+1<argc) parse_int(argv[++i], a.head_dim);
        else if(!std::strcmp(argv[i],"--n-past") && i+1<argc) parse_int(argv[++i], a.n_past);
        else if(!std::strcmp(argv[i],"--repeat") && i+1<argc) parse_int(argv[++i], a.repeat);
        else if(!std::strcmp(argv[i],"--freq-base") && i+1<argc) a.freq_base = std::atof(argv[++i]);
    else if(!std::strcmp(argv[i],"--experimental")) a.experimental = true;
...
std::vector<double> time_loop(int repeat, Fn fn){
    std::vector<double> ns; ns.reserve(repeat);
...
int main(int argc, char** argv){
    ...
    if(args.head_dim %2 !=0){
        std::cerr << "head_dim must be even" << std::endl; return 1;
    }
    ...
    if(args.validate){
        auto validate_init = [&](const char* tag, const std::vector<float>& buf){
            double max_abs=0.0, mean_abs=0.0; double mn=0.0, mx=0.0; bool first=true; bool finite=true;
            for(float v: buf){
                if(!std::isfinite(v)) finite=false;
                double av=std::abs((double)v); if(av>max_abs) max_abs=av; mean_abs+=av;
                if(first){ mn=mx=v; first=false; } else { if(v<mn) mn=v; if(v>mx) mx=v; }
            }
            mean_abs/=buf.size();
    ...
    std::vector<double> base_times, exp_times; base_times.reserve(args.repeat); exp_times.reserve(args.repeat);
    ...
    for(int r=0;r<args.repeat;++r){
        ...
        for(size_t i=0;i<elems;++i){
            ...
            if(!std::isfinite(q_base[i]) || !std::isfinite(k_base[i])) inf_nan_base=1;
            if(!std::isfinite(q_exp[i]) || !std::isfinite(k_exp[i])) inf_nan_exp=1;
        }
