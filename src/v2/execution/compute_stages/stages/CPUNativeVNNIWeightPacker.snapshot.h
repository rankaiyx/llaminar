...
namespace llaminar2::cpu::native_vnni
{
    ...
    inline size_t native_block_bytes_for_codebook(uint8_t codebook_id)
    {
        ...
        case 7:
            return 24; // Q5_1Block
        case 10:
            return 210; // Q6_KBlock (superblock, 256 elements)
        case 11:
            return 110; // Q3_KBlock (superblock, 256 elements)
        case 12:
            return 84; // Q2_KBlock (superblock, 256 elements)
        case 13:
            return 144; // Q4_KBlock (superblock, 256 elements)
        case 14:
            return 176; // Q5_KBlock (superblock, 256 elements)
        case 19:
            return 34; // Q8_0Block
        default:
            return 0;
...
#pragma omp parallel for schedule(static) collapse(2)
            for (int chunk = 0; chunk < N_chunks; ++chunk)
            {
                for (int kb = 0; kb < bpr; ++kb)
                {
                    ...
                    // Extract payload bytes and scales from native blocks
                    // Temp arrays for this chunk/kb pair (stack-allocated, small)
                    uint8_t payload_buf[64][16]; // max payload_bytes = 16 for nibble formats
                    uint16_t scales_buf[64];
                    uint16_t mins_buf[64];
                    ...
                    for (int col = 0; col < n_cols; ++col)
                    {
                        ...
                        const uint8_t *blk = native_blocks + ((size_t)row * bpr + kb) * block_size;

                        // First 2 bytes are FP16 scale (d) for all per-block formats
                        uint16_t d;
                        std::memcpy(&d, blk, 2);
                        scales_buf[col] = d;

                        if (codebook_id == 5) // Q4_1: has min after scale
                        {
                            uint16_t m;
                            std::memcpy(&m, blk + 2, 2);
                            mins_buf[col] = m;
                            std::memcpy(payload_buf[col], blk + 4, pb);
                        }
                        else
                        {
                            // Q4_0 (0), IQ4_NL (4): payload starts at offset 2
                            std::memcpy(payload_buf[col], blk + 2, pb);
                        }
                    }
                    ...
                    for (int group = 0; group < 4; ++group)
                    {
                        for (int z = 0; z < 4; ++z)
                        {
                            ...
                            for (int lane = 0; lane < 16; ++lane)
                            {
                                ...
                                if (col < n_cols)
                                {
                                    zmm_dst[lane * 4 + 0] = payload_buf[col][group * 4 + 0];
                                    zmm_dst[lane * 4 + 1] = payload_buf[col][group * 4 + 1];
                                    zmm_dst[lane * 4 + 2] = payload_buf[col][group * 4 + 2];
                                    zmm_dst[lane * 4 + 3] = payload_buf[col][group * 4 + 3];
                                }
                                else
                                {
                                    zmm_dst[lane * 4 + 0] = 0;
                    ...
                    for (int col = 0; col < 64; ++col)
                    {
                        ...
                        {
                            int8_t decoded[32];
                            decode_native_block(codebook_id, payload_buf[col], decoded);
                            int32_t sum = 0;
                            for (int i = 0; i < 32; ++i)
                                sum += decoded[i];
        ...
        else
        {
            ...
            //
            // Note: Superblock formats (Q6_K, Q3_K) are NOT supported here;
            // they retain permanent interleaved data (no deferred packing).
            // ---------------------------------------------------------------
...
#pragma omp parallel for schedule(static) collapse(2)
            for (int chunk = 0; chunk < N_chunks; ++chunk)
            {
                for (int kb = 0; kb < bpr; ++kb)
                {
                    ...
                    size_t block_offset_ws = ((size_t)chunk * bpr + kb) * meta.interleaved_block_stride;
                    uint8_t *dst = workspace + block_offset_ws;

                    // Extract INT8 values, scales, and mins from native blocks
                    int8_t int8_buf[64][32];
                    uint16_t scales_buf[64];
                    uint16_t mins_buf[64];
                    ...
                    for (int col = 0; col < n_cols; ++col)
                    {
                        ...
                        else if (codebook_id == 6) // Q5_0: scale(2) + qs(16) + qh(4) = 22 bytes
                        {
                            uint16_t d;
                            std::memcpy(&d, blk, 2);
                            scales_buf[col] = d;
                            decode_q5_0_block(blk + 2, int8_buf[col]);
                        }
                        else if (codebook_id == 7) // Q5_1: scale(2) + min(2) + qh(4) + qs(16) = 24 bytes
                        {
                            uint16_t d, m;
                            std::memcpy(&d, blk, 2);
                            std::memcpy(&m, blk + 2, 2);
                            scales_buf[col] = d;
                            mins_buf[col] = m;
                            // Reconstruct Q5_1Block for correct unpack (qh at offset 4, qs at offset 8)
                            Q5_1Block block;
                            block.d = 0;
                            block.m = 0;
                    ...
                    for (int group = 0; group < 8; ++group)
                    {
                        for (int z = 0; z < 4; ++z)
                        {
                            ...
                            for (int lane = 0; lane < 16; ++lane)
                            {
                                ...
                                if (col < n_cols)
                                {
                                    zmm_dst[lane * 4 + 0] = static_cast<uint8_t>(int8_buf[col][group * 4 + 0]);
                                    zmm_dst[lane * 4 + 1] = static_cast<uint8_t>(int8_buf[col][group * 4 + 1]);
                                    zmm_dst[lane * 4 + 2] = static_cast<uint8_t>(int8_buf[col][group * 4 + 2]);
                                    zmm_dst[lane * 4 + 3] = static_cast<uint8_t>(int8_buf[col][group * 4 + 3]);
                                }
                                else
                                {
                                    zmm_dst[lane * 4 + 0] = 0;
                    ...
                    // Write mins inline (if asymmetric, e.g. Q5_1)
                    if (meta.is_asymmetric)
                    {
