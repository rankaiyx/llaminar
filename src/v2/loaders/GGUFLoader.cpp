...
namespace llaminar2
{
    ...
    GGUFLoader::GGUFLoader() : loaded_(false) {}

    bool GGUFLoader::loadModel(const std::string &file_path)
    {
        ...
        if (!file_stream_)
        {
            std::cerr << "[GGUFLoader] Failed to open file: " << file_path << std::endl;
            return false;
        }
        ...
        if (!parseHeader())
        {
            std::cerr << "[GGUFLoader] Failed to parse header" << std::endl;
            return false;
        }

        if (!parseMetadata())
        {
            std::cerr << "[GGUFLoader] Failed to parse metadata" << std::endl;
            return false;
        }

        if (!parseTensorInfo())
        {
            std::cerr << "[GGUFLoader] Failed to parse tensor info" << std::endl;
            return false;
        }
        ...
        if (aligned != cur)
        {
            ...
            if (!file_stream_)
            {
                std::cerr << "[GGUFLoader] Failed to seek to aligned data offset" << std::endl;
                return false;
            }
        }
        ...
        std::cout << "[GGUFLoader] Loaded " << file_path << std::endl;
    ...
    std::shared_ptr<TensorBase> GGUFLoader::loadTensor(const std::string &tensor_name, int device_idx)
    {
        if (!loaded_)
        {
            std::cerr << "[GGUFLoader] Model not loaded" << std::endl;
            return nullptr;
        }
        ...
        if (!info)
        {
            std::cerr << "[GGUFLoader] Tensor not found: " << tensor_name << std::endl;
            return nullptr;
        }
        ...
        if (!file_stream_)
        {
            std::cerr << "[GGUFLoader] Failed to seek to tensor: " << tensor_name << std::endl;
            return nullptr;
        }
        ...
        {
            std::cerr << "[GGUFLoader] Failed to read tensor data: " << tensor_name << std::endl;
            return nullptr;
        }
        ...
        case GGUFTensorType::F16:
            ...
            std::cerr << "[GGUFLoader] F16 not yet implemented for tensor: " << tensor_name << std::endl;
            return nullptr;
        ...
        default:
            std::cerr << "[GGUFLoader] Unsupported tensor type: "
    ...
    bool GGUFLoader::parseHeader()
    {
        ...
        {
            std::cerr << "[GGUFLoader] Invalid magic number (not a GGUF file)" << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "[GGUFLoader] Failed to read version" << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "[GGUFLoader] Failed to read tensor count" << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "[GGUFLoader] Failed to read metadata count" << std::endl;
            return false;
        }

        std::cout << "[GGUFLoader] Header: version=" << model_.version
    ...
    bool GGUFLoader::readString(std::string &str)
    {
        ...
        if (len > 1000000)
        { // 1MB sanity check
            std::cerr << "[GGUFLoader] String length too large: " << len << std::endl;
            return false;
        }
    ...
    bool GGUFLoader::readArray(GGUFValue &value)
    {
        ...
        {
            std::cerr << "[GGUFLoader] Array length too large: " << array_len << std::endl;
            return false;
        }
        ...
        case GGUFValueType::STRING:
            ...
            return true;
        default:
            std::cerr << "[GGUFLoader] Unknown array element type: " << elem_type << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "[GGUFLoader] Failed to read array data" << std::endl;
            return false;
        }
    ...
    bool GGUFLoader::parseMetadata()
    {
        for (uint64_t i = 0; i < model_.metadata_kv_count; ++i)
        {
            ...
            if (!readString(key))
            {
                std::cerr << "[GGUFLoader] Failed to read metadata key " << i << std::endl;
                return false;
            }
            ...
            {
                std::cerr << "[GGUFLoader] Failed to read value type for key: " << key << std::endl;
                return false;
            }
            ...
            else if (value.type == GGUFValueType::STRING)
            {
                ...
                if (str_len > 1000000)
                {
                    std::cerr << "[GGUFLoader] String value too large: " << str_len << std::endl;
                    return false;
                }
            ...
            else
            {
                ...
                case GGUFValueType::FLOAT64:
                    ...
                    break;
                default:
                    std::cerr << "[GGUFLoader] Unknown value type: " << value_type << std::endl;
                    return false;
                }
    ...
    bool GGUFLoader::parseTensorInfo()
    {
        ...
        for (uint64_t i = 0; i < model_.tensor_count; ++i)
        {
            ...
            {
                std::cerr << "[GGUFLoader] Failed to read tensor name " << i << std::endl;
                return false;
            }
            ...
            if (!readValue(n_dims))
            {
                std::cerr << "[GGUFLoader] Failed to read dimensions for: " << tensor.name << std::endl;
                return false;
            }
            ...
            for (uint32_t j = 0; j < n_dims; ++j)
            {
                ...
                {
                    std::cerr << "[GGUFLoader] Failed to read dimension " << j
            ...
            {
                std::cerr << "[GGUFLoader] Failed to read type for: " << tensor.name << std::endl;
                return false;
            }
            ...
            {
                std::cerr << "[GGUFLoader] Failed to read offset for: " << tensor.name << std::endl;
                return false;
            }
            ...
            {
                std::cerr << "[GGUFLoader] Unknown tensor type: " << type_val
                          << " for: " << tensor.name << std::endl;
                return false;
            }
        }
    ...
    void GGUFLoader::extractModelMetadata()
    {
        // Debug: Print all metadata keys
        std::cout << "[GGUFLoader] Available metadata keys:\n";
        ...
        // Debug output
        std::cout << "[GGUFLoader] Extracted metadata:\n";
