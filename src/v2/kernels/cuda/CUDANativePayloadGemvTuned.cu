/**
 * @file CUDANativePayloadGemvTuned.cu
 * @brief Tuned CUDA native-payload GEMV kernels for decode (M=1).
 *
 ...
 *
 * Dispatch is tuned from the automated native-payload sweep harness across all supported
...
namespace
{
    ...
    // =====================================================================
    // Shape classifier — tuned from the native-payload sweep results.
...
#include "kernels/cuda/CUDANativePayloadGemvDispatchHeuristicGenerated.inc"
    ...
    __global__ void nativePayloadGemv_wide(
...
#pragma unroll
            for (int c = 0; c < CPT; ++c)
            {
                ...
                const uint8_t *payload = d_payload + linear *
                                                         llaminar2::cuda_native_payload::payload_bytes_for_codebook<CB>();

                int32_t packed_groups[8];
                llaminar2::cuda_native_payload::decode_groups<CB>(payload, packed_groups);

                if constexpr (llaminar2::cuda_native_payload::CodebookTraits<CB>::is_dual_scale)
                {
...
#pragma unroll
                    for (int g = 0; g < 4; ++g)
                    {
                        ...
                        sum_lo += llaminar2::cuda_native_payload::sum_packed_i8(smem_A[g]);
                        sum_hi += llaminar2::cuda_native_payload::sum_packed_i8(smem_A[g + 4]);
                    }
                    ...
                    if constexpr (llaminar2::cuda_native_payload::CodebookTraits<CB>::is_dual_scale_asym)
                    {
                        const uint32_t emin = d_emins ? d_emins[linear] : 0u;
                        const float min_lo = llaminar2::cuda_native_payload::fp16_bits_to_float(static_cast<uint16_t>(emin));
                        const float min_hi = llaminar2::cuda_native_payload::fp16_bits_to_float(static_cast<uint16_t>(emin >> 16));
                        acc[c] += scale_a * (min_lo * static_cast<float>(sum_lo) + min_hi * static_cast<float>(sum_hi));
                    }

                    if constexpr (llaminar2::cuda_native_payload::CodebookTraits<CB>::is_iq1_m)
                    {
                        constexpr float IQ1S_DELTA = 0.125f;
                        const uint8_t qh0 = payload[4];
                        const uint8_t qh1 = payload[5];
                        const int sg0 = llaminar2::cuda_native_payload::sum_packed_i8(smem_A[0]) + llaminar2::cuda_native_payload::sum_packed_i8(smem_A[1]);
                        const int sg1 = llaminar2::cuda_native_payload::sum_packed_i8(smem_A[2]) + llaminar2::cuda_native_payload::sum_packed_i8(smem_A[3]);
                        const int sg2 = llaminar2::cuda_native_payload::sum_packed_i8(smem_A[4]) + llaminar2::cuda_native_payload::sum_packed_i8(smem_A[5]);
                        const int sg3 = llaminar2::cuda_native_payload::sum_packed_i8(smem_A[6]) + llaminar2::cuda_native_payload::sum_packed_i8(smem_A[7]);
...
#pragma unroll
                    for (int g = 0; g < 8; ++g)
                    {
                        ...
                        sum_a += llaminar2::cuda_native_payload::sum_packed_i8(smem_A[g]);
                    }
                    ...
                    if constexpr (llaminar2::cuda_native_payload::CodebookTraits<CB>::is_asymmetric)
                    {
...
#pragma unroll
    ...
    __global__ void nativePayloadGemv_kpar(
        const int8_t *__restrict__ d_A_int8,
        const uint8_t *__restrict__ d_payload,
        const uint16_t *__restrict__ d_scales,
        const uint16_t *__restrict__ d_mins,
...
#pragma unroll
            for (int c = 0; c < CPT; ++c)
            {
                ...
                if constexpr (TWO_PHASE)
                    llaminar2::cuda_native_payload::decode_groups_vec<CB>(payload, packed_groups);
                else
                    llaminar2::cuda_native_payload::decode_groups<CB>(payload, packed_groups);
...
#pragma unroll
    ...
    // =====================================================================
    __global__ void nativePayloadGemv_reduce(
    ...
    // =====================================================================
    __global__ void nativePayloadGemv_epilogue(
        float *__restrict__ d_C,
        const float *__restrict__ d_C_existing,
        const float *__restrict__ d_bias,
        int N, float beta)
    ...
    {
        constexpr int payload_bytes = llaminar2::cuda_native_payload::CodebookTraits<CB>::payload_bytes;
        ...
        cudaStream_t stream)
    {
        ...
        case WideTile::T32_C1:
        {
            ...
            nativePayloadGemv_wide<32, 1, CB><<<grid_n, 32, 0, stream>>>(
        ...
        case WideTile::T64_C1:
        {
            ...
            nativePayloadGemv_wide<64, 1, CB><<<grid_n, 64, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        ...
        {
            const int grid_n = (N + 64 - 1) / 64;
            nativePayloadGemv_wide<64, 2, CB><<<grid_n, 32, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        case WideTile::T128_C1:
        {
            ...
            nativePayloadGemv_wide<128, 1, CB><<<grid_n, 128, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        ...
        {
            const int grid_n = (N + 128 - 1) / 128;
            nativePayloadGemv_wide<128, 2, CB><<<grid_n, 64, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        case WideTile::T256_C2:
        {
            ...
            nativePayloadGemv_wide<256, 2, CB><<<grid_n, 128, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        ...
        {
            const int grid_n = (N + 256 - 1) / 256;
            nativePayloadGemv_wide<256, 4, CB><<<grid_n, 64, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        case WideTile::T512_C4:
        {
            ...
            nativePayloadGemv_wide<512, 4, CB><<<grid_n, 128, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C, d_scales_A,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        }
        ...
        int device_id, cudaStream_t stream)
    {
        ...
        if (use_two_phase)
        {
            ...
            nativePayloadGemv_kpar<TILE_N, CPT, CB, true><<<grid, THREADS, 0, stream>>>(
            ...
            nativePayloadGemv_reduce<<<rblk, 256, 0, stream>>>(
        ...
        else
        {
            ...
            nativePayloadGemv_kpar<TILE_N, CPT, CB, false><<<grid, THREADS, 0, stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins,
            ...
            if ((beta != 0.0f && d_C_existing) || d_bias)
            {
                ...
                nativePayloadGemv_epilogue<<<eblk, 256, 0, stream>>>(
...
// =========================================================================
// Public API — matches original cudaNativePayloadGemv_fp32 signature
// =========================================================================
extern "C"
{
    bool cudaNativePayloadGemvTuned_supportsCodebook(uint8_t codebook_id)
    {
        switch (codebook_id)
        {
        case 0:
        ...
        void *stream)
    {
        ...
        if (N <= 0 || K <= 0 || (K % BLOCK_K) != 0)
            return false;
        if (!cudaNativePayloadGemvTuned_supportsCodebook(codebook_id))
            return false;
