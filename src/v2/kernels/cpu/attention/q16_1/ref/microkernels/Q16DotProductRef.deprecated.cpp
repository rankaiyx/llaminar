...
namespace llaminar2::kernels::q16_1::microkernels
{
        ...
        int kv_blocks_per_row)
    {
        ...
        for (int b = 0; b < num_blocks; ++b)
        {
            ...
            // Block dot product - use INT64 to avoid overflow!
            // Q16_1 uses int16 quantized values. For 32 elements:
            // max|int16×int16| = 32767² = 1,073,676,289
            // max sum of 32 products = 32 × 1,073,676,289 = 34,357,641,248
            // This exceeds INT32_MAX (2,147,483,647), so we MUST use INT64.
            int64_t block_dot = 0;
            ...
            for (int i = 0; i < elems_in_block; ++i)
            {
                ...
                block_dot += static_cast<int64_t>(q_val) * static_cast<int64_t>(k_val);
            }
        ...
        float *scores)
    {
        ...
        bool do_trace = cfg.enabled && params.q_head == cfg.head;
        ...
        for (int kv = 0; kv < k_count; ++kv)
        {
            ...
            // Trace for k_row=0 to compare with slow path
            if (do_trace && k_row == 0 && q_row == cfg.row)
            {
                ...
                int num_blocks = (params.head_dim + BLOCK_SIZE - 1) / BLOCK_SIZE;
                std::ostringstream block_trace;
                float recomputed_dot = 0.0f;
                for (int b = 0; b < num_blocks; ++b)
                {
                    int q_col_base = q_start_col + b * BLOCK_SIZE;
                    int k_col_base = k_start_col + b * BLOCK_SIZE;
                    ...
                    for (int i = 0; i < elems_in_block; ++i)
                    {
                        block_dot += static_cast<int32_t>(q_block.qs[i]) * static_cast<int32_t>(k_block.qs[i]);
                    }
                ...
                LOG_DEBUG("[Q16AttnDump] FAST_DOT blocks: " << block_trace.str());
                LOG_DEBUG("[Q16AttnDump] FAST_DOT recomputed_dot=" << recomputed_dot);
                ...
                for (int b = 0; b < num_blocks; ++b)
                {
                    ...
                    for (int i = 0; i < elems_in_block; ++i)
                    {
                        float qv = static_cast<float>(q_block.qs[i]) * q_block.d;
                        float kv = static_cast<float>(k_block.qs[i]) * k_block.d;
                        if (elem < 8)
                            first8 << "d" << elem << ":(q=" << qv << ",k=" << kv << ",p=" << (qv * kv) << ") ";
                        if (elem >= params.head_dim - 8)
                            last8 << "d" << elem << ":(q=" << qv << ",k=" << kv << ",p=" << (qv * kv) << ") ";
                        elem++;
                    }
                }
                LOG_DEBUG("[Q16AttnDump] FAST_DOT first8: " << first8.str());
                LOG_DEBUG("[Q16AttnDump] FAST_DOT last8: " << last8.str());
            }
        }
    }
