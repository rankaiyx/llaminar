 ...
 * auto kernel = std::make_unique<CPUQuantisedGemmKernel>(weights_tensor.get());
 *
 * // Basic GEMM: C = A @ W
...
#include "CPUNativeVNNIGemmKernel.h"
...
namespace llaminar2
{
    namespace gemm
    {
         ...
         */
        class CPUQuantisedGemmKernel : public ITensorGemm
        {
        public:
            /**
             ...
             */
            CPUQuantisedGemmKernel(const TensorBase *weights)
            {
                pack_weights_generic(weights);
                initNativeVNNI(weights);
            }
             ...
             */
            CPUQuantisedGemmKernel(const TensorBase *weights, int row_start, int row_end)
            {
                pack_weights_generic(weights, row_start, row_end);
                initNativeVNNI(weights, row_start, row_end);
            }
             ...
             */
            CPUQuantisedGemmKernel(const Q8_1Tensor *weights)
            {
                pack_weights_generic(weights);
            }
             ...
             * @deprecated Use the generic TensorBase constructor instead
             */
            CPUQuantisedGemmKernel(const Q8_0Tensor *weights)
            {
                pack_weights_generic(weights);
            }
             ...
             * @deprecated Use the generic TensorBase constructor instead
             */
            CPUQuantisedGemmKernel(const Q4_0Tensor *weights)
            {
                pack_weights_generic(weights);
            }
             ...
             * @param original_tensor Original tensor for NativeVNNI init (optional, may be nullptr)
             * @note Use this when tensor owns packed data and you want kernel to reference it
             */
            explicit CPUQuantisedGemmKernel(const QuantisedPackedWeights *packed, const TensorBase *original_tensor = nullptr)
                : external_packed_(packed)
            {
                if (original_tensor)
                    initNativeVNNI(original_tensor);
            }
...
#pragma omp simd reduction(+ : sum)
            ...
            void pack_weights_generic(const TensorBase *weights, int row_start = 0, int row_end = -1)
            {
                ...
                {
                    LOG_ERROR("[CPUQuantisedGemmKernel] Failed to pack weights");
                }
            }
        ...
        public:
            ...
            static bool packWeightsInto(const TensorBase *weights, QuantisedPackedWeights &out, int row_start = 0, int row_end = -1)
            {
                ...
                {
                    LOG_ERROR("[CPUQuantisedGemmKernel] Invalid row range: [" << row_start << ", " << row_end << ")");
                    return false;
                }
...
#pragma omp parallel for schedule(static)
        ...
        public:
            ...
            {
                CPUQuantisedGemmKernel *kernel;    ///< Kernel with packed weights [n, k]
             ...
             * auto* kernel_q = static_cast<CPUQuantisedGemmKernel*>(KernelFactory::getOrCreateGemmEngine(prepared_q));
             * auto* kernel_k = static_cast<CPUQuantisedGemmKernel*>(KernelFactory::getOrCreateGemmEngine(prepared_k));
             * auto* kernel_v = static_cast<CPUQuantisedGemmKernel*>(KernelFactory::getOrCreateGemmEngine(prepared_v));
             *
             * CPUQuantisedGemmKernel::multiply_fused_multi(
             *     normalized->data(),
             *     {
        ...
        public:
             ...
             *
             * CPUQuantisedGemmKernel supports optimized fusion: input is quantized once
                ...
                DeviceWorkspaceManager *workspace = nullptr) override
            {
                ...
                {
                    // The kernel in FusedProjectionDesc must be a CPUQuantisedGemmKernel
                    auto *qkernel = dynamic_cast<CPUQuantisedGemmKernel *>(proj.kernel);
                    if (!qkernel)
                    {
                        LOG_ERROR("[CPUQuantisedGemmKernel::multiply_fused] Projection kernel is not CPUQuantisedGemmKernel");
                        return false;
                    }
            ...
            {
                // Route M=1 GEMV to NativeVNNI kernel when available
                if (m == 1 && vnni_gemv_kernel_ && device_idx == -1)
                {
                    return vnni_gemv_kernel_->multiply(A, C, m, n, k, transpose_B, alpha, beta, ctx, device_idx, workspace);
                }

                // M>1 GEMM: use JIT kernel
                (void)transpose_B;
                (void)workspace;
                                ...
                                const float *gate_input = nullptr, bool do_swiglu = false)
            {
                ...
                if (n != pw.N || k != pw.K)
                {
                    LOG_ERROR("Dimension mismatch in CPUQuantisedGemmKernel: expected N=" << pw.N << ", K=" << pw.K
                                                                                          << " but got N=" << n << ", K=" << k);
                    return false;
                }
...
#pragma omp for collapse(2) schedule(static)
            ...
            bool multiply_activations(const float *A, const float *B, float *C, int m, int n, int k, bool transpose_B, float alpha, float beta, const MPIContext *ctx, int device_idx) override
            {
                ...
                LOG_ERROR("[CPUQuantisedGemmKernel] multiply_activations with two activation matrices not supported");
                return false;
            }
             ...
             *
             * CPUQuantisedGemmKernel is designed for weight projections (packed INT8 weights).
            ...
            bool multiply_activations_strided(const float *A, const float *B, float *C, int m, int n, int k, int stride_a, int stride_b, int stride_c, bool transpose_B, float alpha, float beta, const MPIContext *ctx, int device_idx) override
            {
                ...
                (void)ctx;
                (void)device_idx;
                LOG_ERROR("[CPUQuantisedGemmKernel] multiply_activations_strided not supported - use FloatingPointGemmKernel");
                return false;
            }
                ...
                ActivationFormat format_B) override
            {
                ...
                if (!transpose_B)
                {
                    LOG_ERROR("[CPUQuantisedGemmKernel] multiply_with_softmax_strided_typed_impl: transpose_B must be true for attention");
                    return false;
                }
                ...
                {
                    LOG_ERROR("[CPUQuantisedGemmKernel] multiply_with_softmax_strided_typed_impl: requires Q8_1 format, got A="
            ...
            {
                return true; // CPUQuantisedGemmKernel supports Q8_1 activation sharing
            }
                ...
                int activation_row_offset = 0) override
            {
                ...
                if (c_type == TensorType::Q8_1)
                {
                    ...
                    if (a_type == TensorType::Q8_1)
                    {
                        ...
                        {
                            LOG_ERROR("[CPUQuantisedGemmKernel] activation_row_offset not supported for Q8_1 activations");
                            return false;
                        }
...
#pragma omp for schedule(dynamic)
                ...
                const MPIContext *ctx, int device_idx)
            {
                ...
                if (n != pw.N || k != pw.K)
                {
                    LOG_ERROR("Dimension mismatch in CPUQuantisedGemmKernel::multiply_q8_1_direct: "
...
#pragma omp for collapse(2) schedule(static)
            ...
            // NativeVNNI GEMV kernel (M=1 fast path)
            // Initialized when the weight tensor supports NativeVNNI format.
            // Routes M=1 (decode) through the optimized NativeVNNI GEMV path
            // while M>1 (prefill) continues using the JIT GEMM path above.
            std::unique_ptr<cpu::native_vnni::CPUNativeVNNIGemmKernel> vnni_gemv_kernel_;

            /**
             * @brief Initialize NativeVNNI GEMV kernel if the tensor supports it.
             *
             * Checks for VNNI format info and AVX-512 VNNI support.
             * Called from constructors after JIT weight packing.
             */
            void initNativeVNNI(const TensorBase *weights, int row_start = 0, int row_end = -1)
            {
                const auto *unpackable = dynamic_cast<const IINT8Unpackable *>(weights);
                if (!unpackable || !unpackable->vnniFormatInfo())
                    return;
                if (!llaminar2::cpu_supports_avx512_vnni())
                    return;

                vnni_gemv_kernel_ = std::make_unique<cpu::native_vnni::CPUNativeVNNIGemmKernel>(weights, row_start, row_end);
                LOG_DEBUG("[CPUQuantisedGemmKernel] NativeVNNI GEMV kernel initialized for M=1 fast path"
                          << " N=" << packed_weights().N << " K=" << packed_weights().K);
            }
        };
