...
struct TPGemmExecConfig {
    enum class Mode { Column, Row } mode = Mode::Column; // Which output axis is partitioned
    int tp_size = 1;
    int tp_rank = 0;
};
...
public:
    ...
    TPGemmLocalResult run(const float* A, const float* B) const {
        TPGemmLocalResult r; r.partA = specA_; r.partB = specB_;
        if (cfg_.tp_size <= 1) {
            r.M_local = M_; r.N_local = N_; r.buffer.resize(M_*N_);
            if (!fn_(A,B,r.buffer.data(), M_, N_, K_)) throw std::runtime_error("TPGemmExecutor single-partition matmul failed");
            return r;
        }
        if (cfg_.mode == TPGemmExecConfig::Mode::Row) {
            r.M_local = specA_.local_dim; r.N_local = N_;
            const float* A_local = A + specA_.local_offset * K_; // row-major advance
            r.buffer.resize(r.M_local * r.N_local);
            if (!fn_(A_local, B, r.buffer.data(), r.M_local, r.N_local, K_)) throw std::runtime_error("Row-split local gemm failed");
        } else { // Column mode
            r.M_local = M_; r.N_local = specB_.local_dim;
            std::vector<float> B_packed; B_packed.resize(K_ * r.N_local);
            ...
            r.buffer.resize(r.M_local * r.N_local);
            if (!fn_(A, B_packed.data(), r.buffer.data(), r.M_local, r.N_local, K_)) throw std::runtime_error("Column-split local gemm failed");
        }
        return r;
    }
    ...
    static void reconstruct_columns(const std::vector<TPGemmLocalResult>& parts, float* C_full, std::size_t M, std::size_t N) {
        for (const auto &p : parts) {
            if (p.M_local != M) throw std::runtime_error("Column reconstruct: M mismatch");
    ...
    static void reconstruct_rows(const std::vector<TPGemmLocalResult>& parts, float* C_full, std::size_t M, std::size_t N) {
        for (const auto &p : parts) {
            if (p.N_local != N) throw std::runtime_error("Row reconstruct: N mismatch");
